# Naive Bayes Training Guide for Fraud Detection
...
## 🎯 Why Naive Bayes for Fraud Detection?

- ✅ **Extremely fast training** (seconds, not minutes!)
- ✅ **Handles high-dimensional data** well
- ✅ **Probabilistic predictions** - natural fraud scores
- ✅ **Works with small datasets** - efficient learner
- ✅ **Good baseline** for quick prototyping
- ⚠️ **Assumes feature independence** (rarely true in practice)
- ⚠️ **Lower performance** than complex models
- ⚠️ **Sensitive to feature distributions**

**Expected Performance: ROC-AUC 0.65-0.75** (quick baseline)
...
def engineer_features_nb(trans_df, cust_df):
    """
    Feature engineering optimized for Naive Bayes
    
    Key considerations for Naive Bayes:
    - Prefers discrete/categorical features
    - Gaussian NB handles continuous features
    - Binning continuous features can help
    - Feature independence matters (but often violated)
    """
    ...
    # Time of day categories (discrete bins)
    ...
    # Amount categories (quantile-based binning)
    ...
    df['combined_risk_score'] = (
        df['merchant_risk'].map(risk_mapping).fillna(2) +
        df['category_risk'].map(risk_mapping).fillna(2) +
        df['hour_risk'].map(risk_mapping).fillna(2)
    )
...
### **STEP 4: Feature Preparation for Different Naive Bayes Variants**

```python
def prepare_features_for_nb(df):
    """
    Prepare features for different Naive Bayes variants
    
    - GaussianNB: Continuous features (assumes Gaussian distribution)
    - MultinomialNB: Count/frequency features (non-negative)
    - ComplementNB: Imbalanced data with count features
    ...
    # BINARY FEATURES (for BernoulliNB and GaussianNB)
    binary_features = [
    ...
    # 1. For GaussianNB (continuous + binary)
    X_gaussian = df[continuous_features + binary_features].copy()
    
    # 2. For BernoulliNB (binary only)
    X_bernoulli = df[binary_features].copy()
    
    # 3. For MultinomialNB and ComplementNB (encode categorical as counts)
    ...
    # Shift continuous features to be non-negative
...
# We'll use the Gaussian variant for main demonstrations
...
print("\n⚖️ Applying SMOTE + Undersampling...")
...
# Also prepare balanced sets for other variants
balanced_splits = {}
...
# 1. GAUSSIAN NAIVE BAYES (Most common for continuous features)
...
print("   Best for: Count/frequency features (text classification)")
print("   ⏳ Training...")
...
# 4. BERNOULLI NAIVE BAYES (Binary features only)
...
def evaluate_nb_model(model, X_test, y_test, model_name):
    """
    Comprehensive evaluation for Naive Bayes model
    """
    print(f"\n{'='*80}")
...
# Evaluate all models
results['gaussian'] = evaluate_nb_model(
    models['gaussian'], X_test, y_test, "Gaussian Naive Bayes"
)
...
### **STEP 9: Hyperparameter Tuning for Best Variant**
...
print("   (Controls the portion of largest variance added to variances)")

param_grid = {
    'var_smoothing': np.logspace(-12, -6, 20)  # Test 20 values
}
...
print("\n⏳ Running Grid Search (this is fast - <30 seconds)...")
start_time = time.time()
...
```python
def plot_nb_comparison(results, y_test):
    """
    ...
    model_names = ['gaussian', 'complement', 'multinomial', 'bernoulli']
    ...
    for i, (name, color) in enumerate(zip(model_names, colors)):
        if name in results:
            ...
            ax.plot(recall, precision, color=color, lw=2,
                   label=f'{name.capitalize()} (AUC={pr_auc:.3f})')
    ...
    print("\n📊 Comparison plots saved to 'naive_bayes_comparison.png'")
    plt.close()

# Generate comparison plots
plot_nb_comparison(results, y_test)
```

---

### **STEP 11: Feature Importance Analysis (For Gaussian NB)**
...
print("\n💡 Note: Naive Bayes doesn't provide feature importance directly")
print("   We'll analyze feature means and variances per class\n")
...
feature_importance.to_csv('naive_bayes_feature_importance.csv', index=False)
print("\n✅ Feature importance saved to 'naive_bayes_feature_importance.csv'")
```
...
```python
def optimize_threshold_nb(y_test, y_proba):
    """
    ...
    plt.tight_layout()
    plt.savefig('naive_bayes_threshold_optimization.png', dpi=300, bbox_inches='tight')
    print("   ✅ Threshold plot saved to 'naive_bayes_threshold_optimization.png'")
    plt.close()
...
# Optimize threshold for best model (optimized Gaussian)
optimal_threshold, threshold_metrics = optimize_threshold_nb(y_test, optimized_results['y_proba'])
```
...
print("\n✅ Comparison saved to 'naive_bayes_comparison.csv'")
```
...
for name, model in models.items():
    ...
    with open(filename, 'wb') as f:
        pickle.dump(model, f)
    print(f"✅ {name.capitalize()} NB saved to '{filename}'")

# Save optimized model
with open('naive_bayes_optimized_model.pkl', 'wb') as f:
    pickle.dump(best_gnb, f)
print("✅ Optimized Gaussian NB saved to 'naive_bayes_optimized_model.pkl'")

# Save feature names for each variant
for variant_name in ['gaussian', 'bernoulli', 'multinomial']:
    ...
    print(f"✅ {variant_name.capitalize()} features saved to '{filename}'")

# Save optimal threshold
with open('naive_bayes_optimal_threshold.pkl', 'wb') as f:
    pickle.dump(optimal_threshold, f)
print("✅ Optimal threshold saved to 'naive_bayes_optimal_threshold.pkl'")
...
with open('naive_bayes_results_summary.pkl', 'wb') as f:
    pickle.dump(results_summary, f)
print("✅ Results summary saved to 'naive_bayes_results_summary.pkl'")
...
| Model Variant | ROC-AUC | PR-AUC | F1-Score | Speed |
|---------------|---------|--------|----------|-------|
| **Gaussian NB** | 0.68-0.75 | 0.35-0.50 | 0.45-0.60 | ⚡⚡⚡ |
| **Complement NB** | 0.70-0.76 | 0.38-0.52 | 0.48-0.62 | ⚡⚡⚡ |
| **Multinomial NB** | 0.66-0.72 | 0.32-0.48 | 0.42-0.58 | ⚡⚡⚡ |
| **Bernoulli NB** | 0.62-0.68 | 0.28-0.42 | 0.38-0.52 | ⚡⚡⚡ |

### **Training Time:**
- **Total for all 4 variants: < 1 minute** ⚡
- Hyperparameter tuning: ~30 seconds
- **Fastest algorithm in the entire suite!**

### **Key Advantages:**
✅ Extremely fast training and prediction
✅ Low memory footprint
✅ Handles high-dimensional data well
✅ Naturally provides probability estimates
✅ Works well with small datasets
✅ Great for quick baseline

### **Limitations:**
⚠️ Assumes feature independence (rarely true)
⚠️ Lower performance than ensemble methods
⚠️ Sensitive to feature distributions
⚠️ Can be affected by correlated features
⚠️ Not ideal for complex non-linear patterns

---

## 🚀 Running the Complete Script

Save all code to `fraud_detection_naive_bayes.py` and run:

```bash
python fraud_detection_naive_bayes.py
```

**Estimated completion time: < 2 minutes total** ⚡
...
```python
# deploy_naive_bayes.py
import pickle
import numpy as np

# Load best model
with open('naive_bayes_optimized_model.pkl', 'rb') as f:
    model = pickle.load(f)

with open('naive_bayes_optimal_threshold.pkl', 'rb') as f:
    threshold = pickle.load(f)

def predict_fraud_nb(transaction_features):
    """
    Fast fraud prediction using Naive Bayes
    
    Args:
        transaction_features: numpy array with engineered features
    
    Returns:
        dict with fraud probability and classification
    """
    # Get probability
    fraud_prob = model.predict_proba(transaction_features.reshape(1, -1))[0, 1]
    
    # Classify
    is_fraud = fraud_prob >= threshold
    
    # Confidence level
    confidence_score = abs(fraud_prob - 0.5) * 2  # Scale to 0-1
...
# Example usage
# result = predict_fraud_nb(X_test[0])
# print(result)
```
...
| Metric | Logistic Regression | SVM | Naive Bayes | Winner |
|--------|-------------------|-----|-------------|--------|
| **ROC-AUC** | 0.75-0.82 | 0.80-0.86 | 0.68-0.75 | SVM |
| **PR-AUC** | 0.40-0.55 | 0.45-0.65 | 0.35-0.50 | SVM |
| **Training Time** | ~5 min | ~15-20 min | **< 1 min** ⚡ | **NB** |
| **Prediction Speed** | Fast | Fast | **Fastest** ⚡ | **NB** |
| **Memory Usage** | Low | High | **Very Low** | **NB** |
| **Interpretability** | High | Low | Medium | LR |
| **Best Use Case** | Baseline | Complex patterns | **Quick baseline** | - |

**Conclusion**: Naive Bayes is the fastest algorithm but has lower performance. **Perfect for:**
...
### **Issue 1: Poor performance (ROC-AUC < 0.65)**
**Solutions:**
- Use **ComplementNB** instead of GaussianNB (better for imbalanced data)
- Add more categorical/binned features
- Increase SMOTE ratio
- Try different feature engineering strategies

### **Issue 2: "ValueError: negative values in data"**
**Solution:** This happens with MultinomialNB
```python
# Shift features to be non-negative
X_train[X_train < 0] = 0
```

### **Issue 3: Model predicts mostly one class**
**Solution:**
- Check class imbalance handling
- Adjust classification threshold
- Use ComplementNB for severe imbalance

### **Issue 4: Want better performance**
**Solution:** Naive Bayes is a baseline model. Move to:
- XGBoost (ROC-AUC: 0.88-0.93)
- LightGBM (ROC-AUC: 0.87-0.92)
- Random Forest (ROC-AUC: 0.82-0.88)
...
- [ ] Run complete script
- [ ] Check all 4 variants trained successfully
- [ ] Compare performance across variants
- [ ] Save best model (usually Complement or Gaussian)
- [ ] Note training time (should be < 1 minute!)
- [ ] Compare with previous models (LR, SVM)

---

## 🎓 Key Learnings

1. **ComplementNB often wins** for imbalanced fraud detection
2. **Speed is unmatched** - great for rapid experimentation
3. **Feature engineering matters more** than hyperparameter tuning
4. **Binning continuous features** can improve NB performance
5. **Independence assumption** is violated but still works reasonably
6. **Good baseline** before investing time in complex models

---

## 📈 Next Steps

You've now completed 3 models! Here's the ranking so far:

| Rank | Model | ROC-AUC | Training Time |
|------|-------|---------|---------------|
| 🥇 | **SVM** | 0.80-0.86 | 15-20 min |
| 🥈 | **Logistic Regression** | 0.75-0.82 | 5 min |
| 🥉 | **Naive Bayes** | 0.68-0.75 | < 1 min |

### **Recommended Next Algorithm:**
**XGBoost** or **LightGBM** - These will likely give you the **best performance** (ROC-AUC: 0.88-0.93)!

**Let me know which model you want to tackle next!** 🚀

Options:
1. **XGBoost** (best performance, industry standard)
2. **LightGBM** (faster than XGBoost, similar performance)
3. **Random Forest** (interpretable, robust)
4. **Neural Networks** (deep learning approach)
5. **Isolation Forest** (unsupervised anomaly detection)

---

**Pro Tip:** Keep the Naive Bayes model for ensemble stacking later - it provides different perspective than tree-based models! 🎯
